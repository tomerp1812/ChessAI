...
def create_pieces(controller, fen):
    pieces = []
    white_pieces = []
    black_pieces = []
    ...
    for y,row in enumerate(board):
        for x,char in enumerate(row):
            if char != '.':
                ...
                piece_type = pieces_types[char.lower()]
                
                piece_class = pieces_map.get(piece_type)
                if piece_class:
                    position = (x, y)
                    ...
                    piece = piece_class(
                        position,
                        color,
                        controller.pygame.transform.scale(controller.images[image_key], (100,100))
                    )
